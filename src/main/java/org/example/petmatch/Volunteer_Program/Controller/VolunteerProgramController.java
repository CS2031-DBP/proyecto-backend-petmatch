...
public class VolunteerProgramController {
    ...
    public ResponseEntity<List<VolunteerProgramResponseDto>> getAllProgramas() {
        List<VolunteerProgramResponseDto> programas = volunteerProgramService.getAllProgramas().stream().map(programa -> modelMapper.map(programa, VolunteerProgramResponseDto.class)).toList();
    ...
    public ResponseEntity<NewIdDTO> createPrograma(@AuthenticationPrincipal UserDetails userDetails, @RequestBody @Valid VolunteerProgramRequestDto volunteerProgramRequestDto) {
    ...
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<Void> enrollingVolunteerProgram(@PathVariable Long id, @RequestParam Long voluntarioId) {
        volunteerProgramService.enrollingVolunteerPrograma(id, voluntarioId);
        return ResponseEntity.ok().build();
    }

    @DeleteMapping("/{id}/inscripcion")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> unsubscribeVolunteerProgram(@PathVariable Long id, @RequestParam Long voluntarioId) {
        volunteerProgramService.unsubscribeVolunteerProgram(id, voluntarioId);
        return ResponseEntity.noContent().build();
    }

    @DeleteMapping("/alberge/{programaid}")
    @PreAuthorize("hasRole('ALBERGUE')")
    public ResponseEntity<Void> unsubscribeVolunteerProgramOnShelter(@PathVariable Long programaid, @RequestParam Long voluntarioId, @AuthenticationPrincipal UserDetails userDetails) {
    ...
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<Void> cancellInscriptionProgram(@PathVariable Long programaId, @AuthenticationPrincipal UserDetails userDetails) {
    ...
    public ResponseEntity<Void> adminDeletePrograma(@PathVariable Long id) {
        volunteerProgramService.deletePrograma(id);
        return ResponseEntity.noContent().build();
    }
